...
def get_dashboard_number_cards():
	"""Return dashboard number-card counts from My Audits and DGP Cases."""
	...
	# DGP Case counts
	dgp_filters = _get_dgp_dashboard_filters()
...
def can_create_dgp():
	return bool(DGP_CREATE_ROLES.intersection(set(frappe.get_roles(frappe.session.user))))
...
def get_cmg_preview(misconduct_type, severity, occurrence):
	"""Preview CMG mapping before creating DGP Case"""
	from audit_management.audit_management.doctype.dgp_case.dgp_case import get_cmg_mapping
	return get_cmg_mapping(misconduct_type, severity, occurrence)


def _get_dgp_dashboard_filters():
	user = frappe.session.user
	if user == "Guest":
		return {"owner": ""}

	user_roles = set(frappe.get_roles(user))
	if DGP_CREATE_ROLES.intersection(user_roles) or AUDIT_OVERVIEW_ROLES.intersection(user_roles):
		return {}
...
@frappe.whitelist()
def get_dgp_cases(start=0, limit=20, filters=None):
	"""Return paginated DGP Case records."""
	if filters and isinstance(filters, str):
		filters = json.loads(filters)
	
	base_filters = _get_dgp_dashboard_filters()
	if filters:
		base_filters.update(filters)
...
@frappe.whitelist()
def create_dgp_case(emp_branch, employee, misconduct_type, severity, occurrence, case_subject, case_description, department_alignment=None):
	_ensure_dgp_create_role()

	try:
		# Get employee details
		emp = frappe.get_doc("Employee", employee)
		
		doc = frappe.new_doc("DGP Case")
		doc.emp_branch = emp_branch
		...
		return {
			"name": doc.name,
			"redirect_url": f"/app/dgp-case/{frappe.scrub(doc.name)}",
			"message": "DGP Case created successfully.",
		}
	except Exception as e:
		frappe.log_error(frappe.get_traceback(), "DGP Case Creation Failed")
		raise e


def _ensure_dgp_create_role():
	if not can_create_dgp():
		frappe.throw("You are not allowed to create DGP Cases.", frappe.PermissionError)
