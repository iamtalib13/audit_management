    ...
    if (frm.doc.status === "Draft" && is_audit_team) {
        frm.add_custom_button(__('Raise Request'), function() {
            ...
            frappe.prompt([
                ...
                frappe.call({
                    // 🌟 FIX 1: Matched exact python function name "raiserequest" (No underscore)
                    method: "auditmanagement.auditmanagement.doctype.myaudits.myaudits.raiserequest",
                    args: {
                        docname: frm.doc.name,
                        stagename: values.stagename
                    },
    ...
    if (pending_row && frm.doc.status === "Pending") {
        frm.add_custom_button(__("Submit Response"), function () {
            ...
            let d = new frappe.ui.Dialog({
                ...
                primary_action: function (values) {
                    frappe.call({
                        // 🌟 FIX 2: Matched exact python function name "submitresponse"
                        method: "auditmanagement.auditmanagement.doctype.myaudits.myaudits.submitresponse",
                        args: {
                            docname: frm.doc.name,
                            // 🌟 FIX 3: Python expects "responsetext" without underscore
    ...
    if (frm.doc.status === "Pending" && is_audit_team) {
        ...
        if (next_row) {
            frm.add_custom_button(__("Send to {0}", [next_row.stagename || next_row.stage_name]), function () {
                frappe.call({
                    // 🌟 FIX 4: Matched exact python function name "sendtonextstage"
                    method: "auditmanagement.auditmanagement.doctype.myaudits.myaudits.sendtonextstage",
                    args: { docname: frm.doc.name },
                    callback: function (r) {
                        if (r.message) {
                            frappe.show_alert({ message: r.message, indicator: "green" });
